...
Let's recall our tasks from previous days and build upon them! We will need to start a codespace instance by using "Code -> "..." -> New with Options" to pick a lab scenario:
...
We are now ready to create our first job file using Python. There are two options to do this. If you’re new to Nautobot Jobs, which is probably the case for most of us, I’d recommend starting with Option 1. But don’t worry—take a look at Option 2 as well to understand the structure behind it. 
...
>[!IMPORTANT] 
> The location of the file is very important, this is where Nautobot looks for the job files. 
